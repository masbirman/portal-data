...
class DataRequestExport implements FromCollection, WithHeadings, WithMapping, WithTitle
{
    ...
    public function collection()
    {
        switch ($this->dataType) {
            case 'asesmen_nasional':
                ...
                // Get pelaksanaan asesmen data
                return PelaksanaanAsesmen::where('siklus_asesmen_id', $siklusAsesmen->id)
                    ->where('wilayah_id', $this->wilayahId)
                    ->whereHas('sekolah', function ($query) {
                        $query->where('jenjang_pendidikan_id', $this->jenjangPendidikanId);
                    })
                    ->with(['sekolah.jenjangPendidikan', 'wilayah'])
                    ->get();
            
            case 'survei_lingkungan_belajar':
                // TODO: Implement survei lingkungan belajar data export
                return collect([]);
            
            case 'tes_kemampuan_akademik':
                // TODO: Implement TKA data export
                return collect([]);
    ...
    public function map($row): array
    {
        switch ($this->dataType) {
            ...
            case 'survei_lingkungan_belajar':
                // TODO: Implement survei lingkungan belajar mapping
                return [
                    $row->npsn ?? '-',
                    $row->nama ?? '-',
    ...
    {
        $jenjang = \App\Models\JenjangPendidikan::find($this->jenjangPendidikanId);
        return ucfirst(str_replace('_', ' ', $this->dataType)) . ' ' . $this->tahun . ' ' . ($jenjang->nama ?? '');
    }
}
