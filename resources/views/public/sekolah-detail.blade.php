        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('direktori-sekolah.index') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">Direktori Sekolah</a>
                </div>
            </li>
            ...
            <div class="lg:col-span-1 min-h-[280px] relative overflow-hidden">
                @if($sekolah->foto)
                    ...
                    <img src="{{ asset('storage/' . $sekolah->foto) }}" 
                         alt="Foto {{ $sekolah->nama }}"
                         class="w-full h-full object-cover absolute inset-0">
                @else
                    <!-- Placeholder -->
                    <div class="w-full h-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center absolute inset-0">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800 dark:text-white">{{ $sekolah->jenjangPendidikan->nama ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800 dark:text-white">{{ $sekolah->wilayah->nama ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800 dark:text-white">{{ $sekolah->status_sekolah ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                ...
                @if($statistics['total_asesmen'] > 0)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/30 rounded-lg p-4">
                            ...
                            <p class="text-2xl font-bold text-green-800 dark:text-green-200">{{ $formatPercent($statistics['avg_literasi']) }}%</p>
                        </div>
                        <div class="bg-purple-50 dark:bg-purple-900/30 rounded-lg p-4">
                            ...
                            <p class="text-2xl font-bold text-purple-800 dark:text-purple-200">{{ $formatPercent($statistics['avg_numerasi']) }}%</p>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Siklus</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Peserta</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Literasi</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Numerasi</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($assessmentHistory as $history)
                                        <tr>
                                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $history['siklus'] }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ number_format($history['jumlah_peserta']) }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $formatPercent($history['partisipasi_literasi']) }}%</td>
                                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $formatPercent($history['partisipasi_numerasi']) }}%</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        @foreach($nearbySchools as $nearby)
                            <a href="{{ route('direktori-sekolah.show', $nearby) }}" 
                               class="block bg-gray-50 dark:bg-gray-700 rounded-lg p-3 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                                <h3 class="font-semibold text-gray-800 dark:text-white text-sm line-clamp-1">{{ $nearby->nama }}</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ $nearby->kode_sekolah }}</p>
