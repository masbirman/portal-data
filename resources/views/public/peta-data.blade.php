...
<div class="mb-6">
    ...
    <p class="text-gray-600 dark:text-gray-400">Visualisasi data pelaksanaan asesmen per kabupaten/kota di Sulawesi Tengah</p>
</div>
...
<div class="mt-4 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
    ...
    <div class="mb-3">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <div class="flex items-center">
                <div class="w-3 h-3 rounded-full mr-1.5 animate-ping" style="background-color: #3b82f6; animation-duration: 2s;"></div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-indigo-900 text-white">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider" colspan="2">Status Pelaksanaan</th>
                    <th class="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider" colspan="2">Moda Pelaksanaan</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td class="px-4 py-3 whitespace-nowrap">
                        <div class="flex items-center">
                            @if($wilayah['logo'])
                                <img src="{{ asset('storage/' . $wilayah['logo']) }}" alt="{{ $wilayah['nama'] }}" class="w-8 h-8 rounded-full mr-3 object-contain bg-gray-100">
                            @else
                                <div class="w-8 h-8 rounded-full mr-3 bg-indigo-100 flex items-center justify-center">
                                    <span class="text-indigo-600 font-bold text-xs">{{ substr($wilayah['nama'], 0, 2) }}</span>
                                </div>
                            @endif
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $wilayah['nama'] }}</span>
                        </div>
                    </td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 text-center font-medium">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tahun = '{{ $tahun }}';
    const geojsonUrl = '{{ asset('geojson/sulawesi-tengah.geojson') }}';
