<div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 md:p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
                <label for="wilayah"
                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Wilayah</label>
                <select id="wilayah" wire:model.live="wilayahId"
            ...
            <div>
                <label for="jenjang"
                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jenjang</label>
                <select id="jenjang" wire:model.live="jenjangId"
                    class="w-full py-2 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div
                ...
                <div class="p-4 flex flex-col flex-grow">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        <span
                            class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full {{ $school->status_sekolah === 'Negeri' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' }}">
                            {{ $school->status_sekolah ?? '-' }}
                        </span>
                        <span
                            class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200">
    ...
    <div class="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            <span>Menampilkan {{ $schools->firstItem() ?? 0 }} sampai {{ $schools->lastItem() ?? 0 }} dari
                {{ $schools->total() }} hasil</span>
        </div>
            ...
            @if ($schools->hasPages())
                <div class="flex items-center gap-1">
                    ...
                    @if ($schools->onFirstPage())
                        <span
                            class="px-3 py-1.5 text-sm text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-lg cursor-not-allowed">&laquo;</span>
                    @else
                        <button wire:click="previousPage"
                            class="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">&laquo;</button>
                    @endif
                    ...
                    @if ($start > 1)
                        <button wire:click="gotoPage(1)"
                            class="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">1</button>
                        @if ($start > 2)
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <button wire:click="gotoPage({{ $page }})"
                                class="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">{{ $page }}</button>
                        @endif
                    @endfor
                    ...
                    @if ($end < $lastPage)
                        ...
                        <button wire:click="gotoPage({{ $lastPage }})"
                            class="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">{{ $lastPage }}</button>
                    @endif

                    {{-- Next --}}
                    @if ($schools->hasMorePages())
                        <button wire:click="nextPage"
                            class="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">&raquo;</button>
                    @else
                        <span
                            class="px-3 py-1.5 text-sm text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-lg cursor-not-allowed">&raquo;</span>
                    @endif
                </div>
            @endif
        </div>
